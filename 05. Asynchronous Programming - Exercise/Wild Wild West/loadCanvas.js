function loadCanvas(player){
	...
	let audioReload = new Audio('sounds/reload.mp3');
	let audioShoot = new Audio('sounds/shoot.mp3');
	let audioMusic = new Audio('sounds/music.mp3');
    ...
    function drawCanvas(mouseX, mouseY){
		...
		if(!canvasLoaded || player.hasReloaded){
			audioReload.loop = false;
			audioReload.play();
    ...
    $(canvas).click(function(event) {
		//console.log(audioShoot);
		
		if(totalShots == 0 || audioShoot.ended){
			if(player.bullets > 0){
